...
class UI_settings(tk.Toplevel):
    ...
    def __init__(self, ui, settings: config_controller.UserSettings):
        ...
        Args:
            ui (tk.Tk): 親となるTkinterのメインウィンドウインスタンス。
        ...
        # UserSettingsオブジェクトから、UI表示用の生のJSONデータを取得
...
if __name__ == "__main__":
    # このブロックは、UI_settingsクラスを単独でテストするためのものです。
    # 通常のアプリケーションでは、メインのUIクラスから呼び出されます。

    main = tk.Tk() # テスト用のルートTkinterウィンドウを作成
    main.withdraw() # このルートウィンドウは表示しない（設定ウィンドウのみ表示するため）

    # UserSettingsオブジェクトと、生のconfigデータを準備
    app_settings = config_controller.read_configfile("config.json")
    if app_settings is None:
        print("コンフィグファイルの読み込みに失敗しました。")
        # 設定ファイルが見つからないか、読み込みエラーの場合のフォールバック処理
        app_settings = config_controller.UserSettings() # 空のUserSettingsインスタンスを作成
        raw_config = {} # 空の辞書を設定
    else:
        try:
            # UI構築のために、生のJSONデータも読み込む
            # config_controller.read_configfile が既に内部でこれをやっている可能性もあるため、冗長になる場合もある
            with open("config.json", "r", encoding="utf-8") as f:
                raw_config = json.load(f)
        except Exception as e:
            print(f"テスト用の生のconfig.json読み込みエラー: {e}")
            raw_config = {} # エラーの場合のフォールバック

    # UI_settingsウィンドウのインスタンスを作成し表示
    # main (非表示のルートウィンドウ) を親として渡す
    setting_ui = UI_settings(main, settings=app_settings)
    main.mainloop() # Tkinterイベントループを開始（これにより設定ウィンドウが表示され、操作可能になる）